...
double K_gain = 25; // gain for tuning the magnet motion
...
int CCONV SensorChangeHandler(CPhidgetInterfaceKitHandle IFK, void *usrptr, int Index, int Value)
{
    ...
    double potOff1 = 0.002; // 0.006// potentiometer is placed at on offset on both the sides
...
int main(int argc, char** argv){
  ...
  int dyn1pos = 780; // for spaghetti (default = 850)
  ...
  if (argc>1){
    if (strcmp(argv[1],"var") == 0){
      ...
      stiffVar = false;
      dyn1pos = 650;
      K_gain  = 25; // larger values causes lot of oscillations
    }
    if (argc>2){
      if (strcmp(argv[2],"ball") == 0){
        dyn1pos = 650;
        K_gain  = 25; // larger values causes lot of oscillations
      }
    }
  ...
  cout<<"Disconnecting Phidget and Dynamixel"<<endl;
  return 0;
}
